...
import { getChartCode } from '@/app/lib/chartGenerator'
...
export async function POST(request: NextRequest): Promise<NextResponse<ExecuteResponse>> {
  try {
    ...
    // Append chart generation code — hardcoded ggplot2, never generated by AI
    const chartCode = getChartCode(plan)
    if (chartCode) {
      finalScript = finalScript + chartCode
      console.log('[Execute-R] Chart code appended for test:', plan.selectedTest)
    }
    ...
    // One silent retry for cold-start failures
    ...
    // Fire live citation search — fire-and-forget
    if (typeof fetch !== 'undefined') {
      const keyFindings = interpretation.split('\n').filter(Boolean).slice(0, 3).join(' ')
      fetch(`${process.env.NEXT_PUBLIC_APP_URL || 'https://r-research-assistant-vx33.vercel.app'}/api/find-citations`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ plan, keyFindings, rawOutputSnippet: execution.rawOutput.slice(0, 500) }),
      }).catch(() => {})
    }
