...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    if (ownedTeam) {
      // Get members separately to avoid cross-schema join issue
      const { data: members } = await supabaseAdmin
        .from('team_members')
        .select('id, user_id, role, status, joined_at, invited_email')
        .eq('team_id', ownedTeam.id)

      // Get emails for members who have accounts
      ...
      if (memberUserIds.length > 0) {
        const { data: userEmails } = await supabaseAdmin
          ...
          .select('id, email')
          .in('id', memberUserIds)
        emailMap = Object.fromEntries((userEmails || []).map(u => [u.id, u.email]))
      }

      const enrichedMembers = (members || []).map(m => ({
        ...m,
        users: m.user_id && emailMap[m.user_id] ? { email: emailMap[m.user_id] } : null,
      }))

      return NextResponse.json({
        success: true,
        team: { ...ownedTeam, team_members: enrichedMembers },
        role: 'owner',
      })
    }
    ...
    if (membership) {
      ...
      return NextResponse.json({ success: true, team: memberTeam, role: 'member' })
    }
