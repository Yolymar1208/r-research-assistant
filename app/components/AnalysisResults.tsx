...
type TabKey = 'plan' | 'rscript' | 'output' | 'interpretation' | 'references' | 'chart'
...
export default function AnalysisResults({ result, datasetName = 'Dataset', onReportDownload, liveCitations = [], liveCitationsLoading = false }: Props) {
  ...
  const hasChart = !!(result.execution as any)?.chartBase64

  const tabs: { key: TabKey; label: string }[] = [
    ...(hasChart ? [{ key: 'chart' as TabKey, label: '📊 Chart' }] : []),
            ...
            {hasChart ? (
              <div>
                <div className="mb-3 flex items-center justify-between flex-wrap gap-2">
                  <div>
                    <p className="text-xs font-semibold" style={{ color: '#4a6080' }}>
                      Generated by R · ggplot2 · All values computed by R
                    </p>
                    <p className="text-xs" style={{ color: '#8098b8' }}>
                      Right-click or long-press the chart to save as image
                    </p>
                  </div>
                  {chartBlobUrl && (
                    <a
                ...
                {chartBlobUrl ? (
                  ...
                  <div className="rounded-lg p-8 text-center" style={{ background: 'rgba(255,255,255,0.5)', border: '1px solid rgba(180,200,230,0.3)' }}>
                    <p className="text-sm" style={{ color: '#8098b8' }}>Chart is loading or could not be rendered for this analysis.</p>
                    <p className="text-xs mt-1" style={{ color: '#aab4d4' }}>Check the Raw R Output tab for details.</p>
                  </div>
                )}
                <p className="text-xs mt-3" style={{ color: '#8098b8', lineHeight: 1.6 }}>
                  This chart was generated by R using ggplot2 after the statistical analysis completed. Every axis value, label, and data point reflects the actual R output — not an AI-generated estimate. Use the Download button to save a high-resolution PNG for your DOH report or presentation.
                </p>
              </div>
            ) : (
              <p className="text-sm italic" style={{ color: '#8098b8' }}>No chart available for this analysis.</p>
            )}
          </div>
        )}
