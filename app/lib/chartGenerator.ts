...
export function getChartCode(plan: AnalysisPlan): string {
  ...
  switch (plan.selectedTest) {
    ...
    case 'age_sex_pyramid': return chartCode([
      ...
      '  # Prefer original unmodified data — AI scripts may recode sex to 0/1 in clean',
      '  pyr_src <- df',
      '  for (.src in c("data", "dat", "dataset")) {',
      '    if (exists(.src) && is.data.frame(get(.src)) && nrow(get(.src)) > 0) {',
      '      pyr_src <- get(.src); break',
      '    }',
      '  }',
      '  if (is.null(age_col) || !(age_col %in% names(pyr_src)))',
      '    age_col <- c("age","age_years","age_at_illness")[c("age","age_years","age_at_illness") %in% names(pyr_src)][1]',
      '  if (is.null(sex_col) || !(sex_col %in% names(pyr_src)))',
      '    sex_col <- c("sex","gender","sex_of_patient")[c("sex","gender","sex_of_patient") %in% names(pyr_src)][1]',
      '  pyr_src$epi_age_col <- suppressWarnings(as.numeric(pyr_src[[age_col]]))',
      '  pyr_src$epi_sex_col <- as.character(pyr_src[[sex_col]])',
      '  pyr_src$epi_sex_col <- ifelse(tolower(pyr_src$epi_sex_col) %in% c("m","male","lalaki","1"), "Male",',
      '                    ifelse(tolower(pyr_src$epi_sex_col) %in% c("f","female","babae","0"), "Female", pyr_src$epi_sex_col))',
      '  df_c <- pyr_src %>% dplyr::filter(!is.na(epi_age_col), epi_sex_col %in% c("Male","Female"))',
      '  if (nrow(df_c) == 0) stop(paste("No valid age/sex rows. Sex values found:", paste(unique(pyr_src$epi_sex_col)[1:5], collapse=",")))',
      ...
      '  pyr <- df_c %>% dplyr::count(age_group, epi_sex_col) %>%',
      '    dplyr::mutate(n_plot = ifelse(epi_sex_col == "Male", -n, n))',
      '  cat("Pyramid rows:", nrow(pyr), "Male:", sum(pyr$epi_sex_col=="Male"), "Female:", sum(pyr$epi_sex_col=="Female"), "\n")',
      '  p_chart <- ggplot(pyr, aes(x = age_group, y = n_plot, fill = epi_sex_col)) +',
      '    geom_col(width = 0.85, alpha = 0.9) +',
      '    coord_flip() +',
      '    scale_y_continuous(labels = function(x) abs(x),',
      '      limits = c(-max(abs(pyr$n_plot))*1.2, max(abs(pyr$n_plot))*1.2)) +',
      '    scale_fill_manual(values = c("Male"="#2e75b6","Female"="#e8b85c"), name=NULL) +',
      '    labs(title = "Age-Sex Pyramid of Cases",',
      '         subtitle = paste0("Total: ", nrow(df_c), " | Male: ", sum(pyr$epi_sex_col=="Male"), " | Female: ", sum(pyr$epi_sex_col=="Female")),',
