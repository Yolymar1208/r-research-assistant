...
export function getChartCode(plan: AnalysisPlan): string {
  ...
  switch (plan.selectedTest) {
    ...
    case 'moving_average': return chartCode([
      '  date_col <- ' + dv,
      '  common_date_names <- c("date_onset","onset_date","date","period","date_report","date_consult")',
      '  if (is.null(date_col) || length(date_col)==0 || !(date_col %in% names(df)))',
      '    date_col <- common_date_names[common_date_names %in% names(df)][1]',
      '  if (is.null(date_col) || is.na(date_col)) stop("No date column found for moving average")',
      '  df$epi_date_col <- suppressWarnings(as.Date(as.character(df[[date_col]])))',
      '  ma_data <- df %>% dplyr::filter(!is.na(epi_date_col)) %>%',
      '    dplyr::count(epi_date_col) %>% dplyr::rename(date = epi_date_col, cases = n) %>%',
      '    dplyr::arrange(date)',
      '  # Compute 7-day MA manually to avoid dplyr/stats::filter conflict',
      '  n_rows <- nrow(ma_data)',
      '  ma7 <- rep(NA_real_, n_rows)',
      '  for (i in 7:n_rows) { ma7[i] <- mean(ma_data$cases[(i-6):i]) }',
      '  ma_data$ma7 <- ma7',
      '  p_chart <- ggplot(ma_data, aes(x = date)) +',
      '    geom_col(aes(y = cases), fill = "#7c5cff", alpha = 0.45, width = 0.8) +',
      '    geom_line(aes(y = ma7, color = "7-day MA"), linewidth = 1.3, na.rm = TRUE) +',
      '    scale_color_manual(values = c("7-day MA" = "#e8b85c"), name = NULL) +',
      '    scale_x_date(date_labels = "%b %d", date_breaks = "1 week") +',
      '    scale_y_continuous(expand = expansion(mult = c(0, 0.12))) +',
      '    labs(',
      '      title = "Case Counts with 7-Day Moving Average",',
      '      subtitle = paste0("Total cases: ", sum(ma_data$cases), " | Period: ",',
      '        format(min(ma_data$date), "%b %d"), " to ", format(max(ma_data$date), "%b %d, %Y")),',
