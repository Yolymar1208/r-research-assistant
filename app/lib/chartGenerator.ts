...
export function getChartCode(plan: AnalysisPlan): string {
  ...
  switch (plan.selectedTest) {
    ...
    case 'survival_analysis': return chartCode([
      '  date_cols <- names(df)[sapply(names(df), function(x) inherits(df[[x]], "Date") ||',
      '    (is.character(df[[x]]) && sum(!is.na(suppressWarnings(as.Date(df[[x]][!is.na(df[[x]])][1:5])))) >= 3))]',
      '  outcome_col <- names(df)[sapply(names(df), function(x) {',
      '    v <- tolower(as.character(df[[x]])); any(v %in% c("died","dead","recovered","discharged"))})][1]',
      '  if (length(date_cols) >= 2 && !is.na(outcome_col)) {',
      '    df$d1 <- suppressWarnings(as.Date(as.character(df[[date_cols[1]]])));',
      '    df$d2 <- suppressWarnings(as.Date(as.character(df[[date_cols[2]]])));',
      '    df$days <- as.numeric(df$d2 - df$d1)',
      '    df$event <- ifelse(tolower(as.character(df[[outcome_col]])) %in% c("died","dead"), 1, 0)',
      '    df_s <- df %>% filter(!is.na(days), days >= 0, !is.na(event))',
      '    if (requireNamespace("survival", quietly=TRUE)) {',
      '      library(survival)',
      '      km <- survfit(Surv(days, event) ~ 1, data = df_s)',
      '      km_df <- data.frame(time=km$time, surv=km$surv, upper=km$upper, lower=km$lower)',
      '      p_chart <- ggplot(km_df, aes(x=time, y=surv)) +',
      '        geom_step(color="#7c5cff", linewidth=1.2) +',
      '        geom_ribbon(aes(ymin=lower, ymax=upper), fill="#7c5cff", alpha=0.15) +',
      '        scale_y_continuous(limits=c(0,1), labels=percent_format()) +',
      '        labs(title="Kaplan-Meier Survival Curve", x="Days from Onset",',
      '             y="Survival Probability", caption="JOANResearchOS | All values computed by R") +' + THEME_BASE,
      '    } else stop("survival package not available")',
      '  } else stop("Could not identify date/outcome columns")',
    ].join('\n'))
